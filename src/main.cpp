...
#if defined(NDEBUG)
# error "Dorfcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x5a89baa7d3840f4be0b06afa135c77cf5ae8b526e45dc8b9349b8425d0fba406");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Dorfcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Dorfcoin Signed Message:\n";
...
{
    // Dorfcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 54 * 60;
static const int64 nTargetSpacing = 9 * 60;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Dorfcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Dorfcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1498067409, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "17June2017 king of the mountainhomes and lord of the cotton candy";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("041e72d76484bef0409f5dd049da82f396c89b766aa170e44eff6309654de53f984dcd7febe5e255d749fede5b7d7022a198e2eb03a53e6f3e3d93f9d6727db775") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1498067409;
        ...
        assert(block.hashMerkleRoot == uint256("0x7b511b9ddba044f6df35ab9976aaf623ceabf446cc2fa8b16875de7102ac6cd4"));
...
unsigned char pchMessageStart[4] = { 0x35, 0x74, 0x94, 0x59 }; // Dorfcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DorfcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DorfcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DorfcoinMiner(CWallet *pwallet)
{
    printf("DorfcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("dorfcoin-miner");
    ...
    try { loop {
        ...
        printf("Running DorfcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DorfcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DorfcoinMiner, pwallet));
}
